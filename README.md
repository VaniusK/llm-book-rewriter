...
*   **Data Privacy (Google):** When making requests from countries in the European Economic Area (EEA) (**including the Netherlands**), Switzerland, or the UK, Google, according to its policy, **does not use your data for model training.**
...
*   **Google Docs Change Tracking (for DOCX):** If you want to compare the original `.docx` and the processed file using the 'Compare documents' feature in Google Docs, the standard method (uploading both) might not show changes correctly. For reliable comparison:
...
*   **Blocking Messages from Google (Filter):** Google Gemini's safety filter was triggered. See the "Important Notes" section. Try changing the prompt, using a different model, or provider (OpenRouter).
