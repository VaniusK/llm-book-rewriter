...
class TestBookProcessor(unittest.TestCase):
    ...
    @patch.dict(config, {"processing": {"output_dir": "test_output", "workers_amount": 3}}, clear=True)
    ...
    def test_heuristics(self):
        ...
        self.assertEqual(heuristic_applier.preprocessing_replace_tags_with_placeholder("<br> text <image/> <br>"), ["@ text @ @", {"replaced_tags": ["<br>", "<image/>", "<br>"], "placeholder": "@"}])
        self.assertEqual(heuristic_applier.postprocessing_replace_tags_with_placeholder("@ text @ @", {"replaced_tags": ["<br>", "<image/>", "<br>"], "placeholder": "@"}), "<br> text <image/> <br>")
